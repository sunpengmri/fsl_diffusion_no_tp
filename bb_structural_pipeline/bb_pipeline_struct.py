...
def bb_pipeline_struct(subject, runTopup, fileConfiguration):
    ...
    if (not 'T1' in fileConfiguration):    
        ...
        # Topup
        if runTopup:
            jobPREPAREFIELDMAP = LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 5 -N "bb_prepare_struct_fieldmap_' \
                + subject + '" -l ' + logDir + ' -j ' + jobMERGE + ',' + \
                ' $BB_BIN_DIR/bb_structural_pipeline/bb_prepare_struct_fieldmap ' + subject )
