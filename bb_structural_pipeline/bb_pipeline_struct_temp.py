...
def bb_pipeline_struct(subject, runTopup, fileConfiguration):
    ...
    if (not 'T1' in fileConfiguration):    
        ...
        if runTopup:
            for encDir in ['AP', 'PA']:
                ...
                jobGETB01 = LT.runCommand(logger,   '${FSLDIR}/bin/fsl_sub -T 5  -N "bb_get_b0s_1_' + subject + '" -l ' + logDir \
                    + ' $BB_BIN_DIR/bb_structural_pipeline/bb_get_b0s.py -i ' + subject + '/dMRI/raw/' + encDir + \
                         '.nii.gz -o ' + subject + '/fieldmap/total_B0_' + encDir + '.nii.gz -n ' + str(numVols) + ' -l ' + str(b0_threshold) )
                jobsB0.append(LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 20 -N "bb_choose_bestB0_1_' + subject \
                    + '" -l ' + logDir + ' -j ' + jobGETB01 + ','  \
                    + ' $BB_BIN_DIR/bb_structural_pipeline/bb_choose_bestB0 ' + subject + \
                    '/fieldmap/total_B0_' + encDir + '.nii.gz ' + subject + '/fieldmap/B0_' + encDir + '.nii.gz '))

            jobMERGE = LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 5 -N "bb_fslmerge_' \
                + subject + '" -j ' + ",".join(jobsB0) +' -l ' + logDir \
                + ' ${FSLDIR}/bin/fslmerge -t ' + subject + '/fieldmap/B0_AP_PA ' + subject \
                    + '/fieldmap/B0_AP ' + subject + '/fieldmap/B0_PA')
        ...
        # Topup
        if runTopup:
            jobPREPAREFIELDMAP = LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 10 -N "bb_prepare_struct_fieldmap_' + subject + '" -l ' + logDir + ' -j ' + jobMERGE + ',' + ' $BB_BIN_DIR/bb_structural_pipeline/bb_prepare_struct_fieldmap ' + subject )
            jobTOPUP = LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 90 -N "bb_topup_' + subject + '" -l ' + logDir + \
                 ' -j ' + jobPREPAREFIELDMAP + ',' + ' ${FSLDIR}/bin/topup --imain=' \
                + subject + '/fieldmap/B0_AP_PA --datain=' + subject + '/fieldmap/acqparams.txt --config=b02b0.cnf --out=' + subject + '/fieldmap/fieldmap_out --fout=' + subject + '/fieldmap/fieldmap_fout --jacout=' + subject + '/fieldmap/fieldmap_jacout -v')
        ...
        if not runTopup:
            ...
            # 
            # jobPOSTTOPUP = LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 60 -N \
            #     "bb_post_topup_' + subject + '" -l ' + logDir + ' -j ' + jobTOPUP + ',' \
            #         + jobSTRUCTINIT + ',' + jobSWI + ' $BB_BIN_DIR/bb_structural_pipeline/bb_post_topup ' + subject )
            jobPOSTTOPUP = LT.runCommand(logger, '${FSLDIR}/bin/fsl_sub -T 60 -N "bb_post_topup_' + \
                subject + '" -l ' + logDir + ' -j ' + jobTOPUP + ',' + jobSTRUCTINIT + ',' \
